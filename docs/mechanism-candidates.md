...
| 16 | Output Schema 结构化抽取（Search→Extract→Normalize→Reason） | search-orchestrator/survey.md §9 + decisions/D-2026-06-24-search-evaluate-p5-output-schema.md（superseded）+ decisions/D-2026-06-25-search-redesign-p5-evidence-map.md（proposed）+ experiments/run-9b-p5-output-schema-v2.md（3/5 有条件）+ run-9b-external-review.md（外部评审决策 C）+ run-9c-p5-output-schema-v3.md（2/5 双盲证伪）+ experiments/run-13-p5-evidence-map.md（2/5 双盲证伪） | A | search-orchestrator skill v2：P5 v1 字段对齐 schema（Run #9c 证伪）与 v2 Evidence Map / Claim Graph（Run #13 证伪）均未对自由文本展现决定性优势；仅 Gap Ledger 强制证据缺口枚举显示窄增量 | **候选** — Run #9c 2/5 双盲证伪字段对齐 schema（Conflict ID Δ=-20%）；Run #13 2/5 双盲证伪 Evidence Map / Claim Graph（Material Relation Recall Δ=+6.3% < +15%，Cross-Dimension 双方 12/12 天花板，安全指标全 0）。两代结构化中间表示均未通过；后续若再评估只验证“追加 Gap Ledger / 证据缺口枚举”最小机制，不再做完整 Evidence Map，未通过前不进入 SKILL.md |
